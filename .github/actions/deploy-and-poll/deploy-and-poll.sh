...
CAPABILITY='sanitized-deployment-diagnostics-v1'
  ...
  while true; do
    ...
    case "$status" in
      ...
      FAILED)
        ...
        diagnostic_code="$(curl --silent --show-error --connect-timeout 10 --max-time 30 --output "$diagnostic_file" --write-out '%{http_code}' "https://$host/deployment-diagnostics?run_id=$(urlencode "$run_id")" 2>/dev/null)"
        if [[ "$diagnostic_code" == 200 ]] && jq empty "$diagnostic_file" >/dev/null 2>&1; then
          diagnostic="$(jq -r '(.diagnostics // []) | join("; ")' "$diagnostic_file")"
        fi
        rm -f "$diagnostic_file"
        write_evidence FAILED "$run_id" "$phase" "$step" "$exit_code" "$error_code" "$diagnostic"
        rm -f "$status_file"
        all_success=false
        break
        ;;
